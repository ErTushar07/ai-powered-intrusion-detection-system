...
def auto_ban_ip(source_ip):
    ...
    if system_os == "linux":
        ...
        try:
            # Using netsh to add a firewall rule
            rule_name = f"IDS_BLOCK_{source_ip}"
            cmd = ["netsh", "advfirewall", "firewall", "add", "rule", 
                   f"name={rule_name}", "dir=in", "action=block", f"remoteip={source_ip}"]
            subprocess.run(cmd, check=True)
            print(f"[!] IPS ACTION: Banned {source_ip} via Windows Firewall.")
            return f"Banned {source_ip} (Windows/netsh)"
        except Exception as e:
            return f"IPS Error: {e} (Admin privileges likely required)"
